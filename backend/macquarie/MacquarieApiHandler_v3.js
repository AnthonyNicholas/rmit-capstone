...
class MacquarieApiHandler3{
    ...
    /**
     * The macquarie API has its own UI we can use to gain access to customer transactions
     * This makes it easy for us, we can just shunt the user to Macquarie, and wait for the callback
     */
    providerInitialHandshake(req, res){
        res.redirect(this.base_url+'/user-interface/login?client_id='+this.mac_client_id);
        return;
    }

    /**
     * Process a callback from a bank API and pass client onto our end client
     * 
     */
    clientRedirect(req, res){
        res.redirect('http://terra.bbqsuitcase.com:3000');
        return;
    }
...
module.exports = MacquarieApiHandler3;
